(function() {
  ...
  var discordLink = document.getElementById('discordButton');

  // Variable to track whether the animation has been executed
  var animationExecuted = true;

  if (nameElement) {
      let isTextChanged; // Declare isTextChanged here

      function updateTypedStrings() {
          isTextChanged = !isTextChanged; // Toggle isTextChanged
          return isTextChanged ? ['luka13_'] : ['Luka Volk'];
      }
      ...
      discordLink.addEventListener('click', function (event) {
        ...
        // Execute the animation only once
        if(animationExecuted){
          new Typed(nameElement, {
              strings: updateTypedStrings(),
              typeSpeed: 100,
              backSpeed: 50,
              showCursor: false,      
          });
